...
export class AssetsResource {
  ...
  /**
   * Rebuilds the 3D scene from the IFC already stored for this asset.
   *
   * The stored scene is a snapshot of the converter that produced it, so a
   * model uploaded before a converter change keeps the old scene until it is
   * regenerated. Returns immediately with `status: "processing"`.
   */
  regenerateModel(assetId: string): Promise<BuildingModel> {
    return this.request(
      "POST",
      `/assets/${encodeURIComponent(assetId)}/model/regenerate`,
    );
  }
