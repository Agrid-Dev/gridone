...
EXPECTED = {
    ...
    "Production Froid": Expected((21, 15, 1), {"symbols": 5, "pipes": 6, "flow": 2}),
}
...
def test_no_link_names_a_plate(plate):
    ...
    off-page pipes, so a link is always a pipe leaving the drawing. The
    committed file never names the plate it leads to: a synoptic id is the
    instance's, minted when a plate is pushed there, so the two productions'
    links to each other are set on the instance after both plates have an id,
    and the files stay portable."""
