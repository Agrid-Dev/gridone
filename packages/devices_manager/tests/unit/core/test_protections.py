...
def policy():
    ...
    return ProtectionGuard(provider, inspector, resolver), provider, inspector, resolver
...
@pytest.mark.parametrize(
    ...
    [
        ...
        (PointObservation(value=True, validity="known"), "protection_blocked"),
        (PointObservation(value=False, validity="unknown"), "protection_unknown"),
        (PointObservation(validity="invalid"), "protection_reference_invalid"),
    ],
)
def test_cross_device_decisions(policy, observation, code):
    ...
    if code:
        assert result.reasons[0].protection_id == "rule"
    assert result.can_confirm_protections == (code == "protection_unknown")
...
def test_target_type_drift_cannot_silently_stop_matching_a_protection(policy):
    ...
    assert result.reasons[0].code == "protection_reference_invalid"
    assert not result.can_confirm_protections
    resolver.assert_not_called()
...
def test_acknowledgement_cannot_override_another_known_prohibition(policy):
    ...
    confirmation = ProtectionConfirmation(
        binding=before.protection_binding,
        protection_ids=before.unknown_protection_ids,
        actor_id="operator",
    ...
    assert refused.reasons[0].code == "protection_blocked"
    assert refused.warnings[0].code == "protection_unknown"
...
def test_changed_rules_and_new_unknowns_cannot_reuse_confirmation(policy):
    ...
    confirmation = confirmation.model_copy(update={"protection_ids": ["rule"]})
...
def test_provider_failure_and_evaluation_limits_fail_closed(policy, monkeypatch):
    ...
    result = guard.evaluate("a", "command", WriteEvaluation(eligible=True, value=True))
    assert result.reasons == [WriteReason(code="protection_unavailable")]
    provider.for_target.side_effect = None
    monkeypatch.setattr("devices_manager.core.protections.MAX_RULES", 0)
    assert (
    ...
    monkeypatch.setattr("devices_manager.core.protections.MAX_RULES", 64)
    monkeypatch.setattr("devices_manager.core.protections.MAX_DEVICE_OPERATIONS", 1)
    assert (
        guard.evaluate("a", "command", WriteEvaluation(eligible=True, value=True))
        .reasons[0]
        .code
