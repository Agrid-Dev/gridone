...
class TestValidateTimeseriesPush:
    @pytest.fixture
    def virtual_dm(self) -> DevicesManager:
        device = VirtualDevice(
            ...
            attributes={
                ...
                "setpoint": Attribute.create(
                    "setpoint", DataType.FLOAT, {"read", "write"}
                ),
                "active": Attribute.create("active", DataType.BOOL, {"read"}),
            },
        )
        return DevicesManager(devices={device.id: device}, drivers={}, transports={})

    def test_success_does_not_raise(self, virtual_dm):
        virtual_dm.validate_timeseries_push(
            "vd1",
            [("temperature", 22.5), ("setpoint", 20.0)],
        )

    def test_success_accepts_read_only_attributes(self, virtual_dm):
        virtual_dm.validate_timeseries_push("vd1", [("temperature", 10.0)])

    def test_unknown_attribute_raises_invalid_error(self, virtual_dm):
        with pytest.raises(InvalidError, match="nonexistent"):
            virtual_dm.validate_timeseries_push("vd1", [("nonexistent", 1.0)])

    def test_wrong_type_raises_invalid_error(self, virtual_dm):
        with pytest.raises(InvalidError):
            virtual_dm.validate_timeseries_push("vd1", [("temperature", "not-a-float")])

    def test_rejects_on_first_bad_point(self, virtual_dm):
        """Batch is rejected at first failing point."""
        with pytest.raises(InvalidError):
            virtual_dm.validate_timeseries_push(
                "vd1",
                [("temperature", 20.0), ("nonexistent", 1.0)],
            )

    def test_bool_type_validates_correctly(self, virtual_dm):
        virtual_dm.validate_timeseries_push("vd1", [("active", True)])

    def test_unknown_device_raises_not_found(self, virtual_dm):
        with pytest.raises(NotFoundError):
            virtual_dm.validate_timeseries_push(
                "no-such-device", [("temperature", 1.0)]
            )

    def test_empty_points_does_not_raise(self, virtual_dm):
        virtual_dm.validate_timeseries_push("vd1", [])

    def test_physical_device_raises_invalid_error(self, device):
        dm = DevicesManager(devices={device.id: device}, drivers={}, transports={})
        with pytest.raises(InvalidError):
            dm.validate_timeseries_push(device.id, [])

    @pytest.mark.parametrize(
        ("attr_name", "value"),
        [
            ("temperature", 22.5),
            ("temperature", 22),  # int coerced to float
            ("active", True),
        ],
    )
    def test_valid_types_parametrized(self, virtual_dm, attr_name, value):
        virtual_dm.validate_timeseries_push("vd1", [(attr_name, value)])
