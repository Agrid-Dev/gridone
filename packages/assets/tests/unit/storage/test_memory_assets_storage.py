...
class TestBuildingModelStorage:
    ...
    async def test_set_model_result_persists_converter_version(self):
        storage = MemoryAssetsStorage()
        await storage.save_model(_model(), b"ifc")
        ready = _model().model_copy(
            update={
                "status": BuildingModelStatus.READY,
                "converter_version": 7,
    ...
    async def test_list_stale_ready_model_ids_only_returns_older_ready(self):
        storage = MemoryAssetsStorage()
        for asset_id, status, version in (
            ...
            await storage.save_model(_model(asset_id), b"ifc")
            await storage.set_model_result(
        ...
        assert await storage.list_stale_ready_model_ids(3) == ["stale"]
