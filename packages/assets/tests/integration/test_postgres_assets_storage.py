...
class TestBuildingModels:
    ...
    async def test_converter_version_round_trips_and_lists_stale(
        ...
        for asset_id, version in (("old", 1), ("current", 3)):
            await self._seed_building(storage, asset_id)
            await storage.save_model(self._model(asset_id), b"x")
            await storage.set_model_result(
                self._model(asset_id).model_copy(
                    update={
                        "status": BuildingModelStatus.READY,
        ...
        old_meta = await storage.get_model("old")
        assert old_meta is not None
        assert old_meta.converter_version == 1
        assert await storage.list_stale_ready_model_ids(3) == ["old"]
