...
class TestUpsertPoints:
    ...
    async def test_command_id_stored_and_fetched(self, storage):
        await storage.create_series(_make_series())
        now = datetime.now(tz=UTC)
        # Insert a command row directly to get a valid FK reference.
        cmd_id = await storage._pool.fetchval(  # noqa: SLF001
        ...
        point = DataPoint(timestamp=now, value=1.0, command_id=cmd_id)
        await storage.upsert_points(KEY, [point])
        fetched = await storage.fetch_points(KEY)
        assert fetched[0].command_id == cmd_id
    ...
    async def test_coalesce_preserves_command_id_when_absent(self, storage):
        ...
        await storage.upsert_points(
            KEY, [DataPoint(timestamp=now, value=1.0, command_id=cmd_id)]
        )
        await storage.upsert_points(KEY, [DataPoint(timestamp=now, value=2.0)])
        fetched = await storage.fetch_points(KEY)
        assert fetched[0].command_id == cmd_id
    ...
    async def test_command_id_overwritten_when_provided(self, storage):
        await storage.create_series(_make_series())
        now = datetime.now(tz=UTC)
        cmd1_id = await storage._pool.fetchval(  # noqa: SLF001
            """INSERT INTO unit_commands
            (batch_id, device_id, attribute, value, data_type, status,
             status_details, user_id, created_at)
            VALUES (NULL, 'd1', 'mode', 'auto', 'str', 'success', NULL, 'u1', $1)
        ...
        cmd2_id = await storage._pool.fetchval(  # noqa: SLF001
            """INSERT INTO unit_commands
            (batch_id, device_id, attribute, value, data_type, status,
             status_details, user_id, created_at)
            VALUES (NULL, 'd1', 'mode', 'auto', 'str', 'success', NULL, 'u1', $1)
        ...
        await storage.upsert_points(
            KEY, [DataPoint(timestamp=now, value=1.0, command_id=cmd1_id)]
        )
        await storage.upsert_points(
            KEY, [DataPoint(timestamp=now, value=1.0, command_id=cmd2_id)]
        )
        fetched = await storage.fetch_points(KEY)
        assert fetched[0].command_id == cmd2_id


class TestRenameSeries:
    async def test_rename_updates_metric(self, storage):
        await storage.create_series(_make_series())
        renamed = await storage.rename_series(KEY, "temp")
        assert renamed is not None
        assert renamed.metric == "temp"

    async def test_rename_preserves_points(self, storage):
        await storage.create_series(_make_series())
        now = datetime.now(tz=UTC)
    ...
    async def test_rename_old_key_no_longer_resolves(self, storage):
        await storage.create_series(_make_series())
        await storage.rename_series(KEY, "temp")
        assert await storage.get_series_by_key(KEY) is None

    async def test_rename_unknown_key_returns_none(self, storage):
        unknown = SeriesKey(owner_id="y", metric="z")
        assert await storage.rename_series(unknown, "temp") is None

    async def test_rename_collision_raises(self, storage):
        await storage.create_series(_make_series())
    ...
    async def test_rename_to_same_name_is_noop_ok(self, storage):
        await storage.create_series(_make_series())
        now = datetime.now(tz=UTC)
        await storage.upsert_points(KEY, [DataPoint(timestamp=now, value=23.5)])
