...
def update_device(
    ...
    if payload.transport_id is not None:
        raise NotImplementedError("Updating transport not yet supported")
    if payload.driver_id is not None:
        raise NotImplementedError("Updating driver not yet supported")
    if payload.name is not None:
        device.name = payload.name
    if payload.config is not None:
        _validate_device_config(payload.config, device.driver.device_config_required)
