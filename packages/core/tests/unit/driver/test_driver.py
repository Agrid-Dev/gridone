...
class TestDriverAttachUpdater:
    def test_attach_updater_success(self, driver, mock_transport):
        ...
        driver.attach_updater("temperature", device_config, callback)

        # Simulate transport reading a value
        handler = mock_transport._read_handlers.get("GET /temperature")
    ...
    def test_attach_updater_invalid_attribute(self, driver):
        device_config = {"device_id": "device1"}
        ...
        with pytest.raises(ValueError, match="Attribute invalid_attr is not supported"):
            driver.attach_updater("invalid_attr", device_config, callback)
