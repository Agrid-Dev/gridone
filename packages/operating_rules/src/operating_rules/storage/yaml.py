...
class YamlStorage(MemoryStorage):
    def __init__(self, root: str) -> None:
        super().__init__()
        self._path = Path(root) / "operating_rules.json"
        self._path.parent.mkdir(parents=True, exist_ok=True)
        if self._path.exists():
            self._revisions = TypeAdapter(dict[str, list[OperatingRule]]).validate_json(
    ...
    async def save(self, operating_rule: OperatingRule) -> OperatingRule:
        ...
        saved = await super().save(operating_rule)
        ...
        try:
            with tempfile.NamedTemporaryFile(
                ...
                stream.write(
                    TypeAdapter(dict[str, list[OperatingRule]]).dump_json(
                        self._revisions
                    )
                )
                stream.flush()
