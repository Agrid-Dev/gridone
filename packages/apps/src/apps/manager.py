...
from apps.storage import build_app_storage
...
class AppsManager:
    ...
    async def from_storage(
        ...
        app_storage = await build_app_storage(storage_url)
        return cls(app_storage, users_manager)
    ...
    async def create_app(
        ...
        """Create a service-account user and a registered app from manifest data.

        Intended to be called after a registration request has been accepted.
        """
        user = await self._users_manager.create_user(
            ...
            pre_hashed_password=hashed_password,
        )
