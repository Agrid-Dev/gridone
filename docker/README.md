...
| `GRIDONE_ADMIN_PASSWORD` | _(required on first boot)_ | Password for the `admin` account, seeded **only** on first boot against an empty database. The service refuses to start against an empty database without it |
