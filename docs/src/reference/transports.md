...
**Lost session** — when the broker drops the connection (a broker restart, a network cut), the message loop ends: the transport moves to `connection_error` and reconnects on its own, without waiting for a read: at once, then after 1 s, 2 s, 4 s… up to 60 s between attempts while the broker stays unreachable. The new session subscribes again to every topic still listened to, since a broker forgets the subscriptions of a session it dropped. Closing the transport (deleting it, stopping the service) ends the attempts. Devices using the transport are then told of the reconnection (see [acquiring command dependencies](driver-schema/update-strategy.md#acquiring-command-dependencies)).
