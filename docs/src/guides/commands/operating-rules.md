...
An operating rule refuses a requested write when its condition is not met. It never
sends a stop. Administrators manage `/operating-rules/`; rules apply to UI, API, CLI,
automations and grouped commands.

In the UI, open a device, select **Configuration**, then **OperatingRules**. Administrators can
create an operating rule, choose a writable attribute and the requested value, then
...
The tab lists active and retired operating rules targeting that device. Open a
operating rule to inspect its references and revision history, edit it, or retire it
with a reason. Operators and viewers can inspect operating rules without changing
...
Use `POST /operating-rules/` to prevent starting A unless B is observed stopped:
...
`GET /operating-rules/` includes reference diagnostics; pass `device_id` to filter by
the protected device. `GET /operating-rules/schema` supplies the definition and
retirement form schemas. `GET /operating-rules/{id}/history`
returns every revision. Replace with `PUT /operating-rules/{id}`, including the current
`revision`. Retire with `POST /operating-rules/{id}/retire`, including `revision` and a
