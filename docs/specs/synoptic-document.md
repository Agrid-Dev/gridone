...
- **Two links per change-over block.** A `link` has one inlet and one outlet a cell apart; the cold supply joins the hot supply riser and the cold return leaves the hot return riser two columns away, so one link would cross a riser. Each leg has its own link, as the bays' cold water has. The five links to the cold production (the balance line at the vessel and the four change-over legs) lead to the fourth plate, and the fourth plate's balance line leads back here; the committed files name no target, since a synoptic id is the instance's, so the two are pointed at each other on the instance once both plates are pushed.
...
| Element | Slot / tag | Device | Attribute | Format |
|---|---|---|---|---|
| `pompe-peg-e2-a`, `pompe-peg-e2-b` | `state`, `speed` | `schneider_pompe_peg_e2a` / `_e2b` | `onoff_state`, `speed` | MARCHE / ARRÊT; tr/min, 0. Head B holds the drive's last reported speed (2358) while stopped; raw on the chip |
...
| `cpt-eg-ech-04` | `energy` | none bound; `device_id` is the meter | the meter's `energie` has one point since June (339 169 984, Wh by the same inference as the hot meter) while its power and flow read live; frozen or a register the driver does not declare | `text`, "non mesurée" |
...
- **What it could not author, added by hand**: pipe tags (the twelve readings, four of them bound), the title label, the description, readable ids (the editor numbers by type: `chilled_supply-5`).
