...
- **The editor authored 21 symbols, 15 runs, 5 attribute bindings, 8 text markers, 3 device ids and 2 flows, and the API took the document first time.** Every waypoint the hand-written third plate carries, the editor produced from the same clicks, overhead crossings included; the collector port form and the attribute picker bound what the hand pass would have typed.
- **What it could not author, added by hand**: pipe tags (the twelve readings, six of them bound), the title label, the description, readable ids (the editor numbers by type: `chilled_supply-5`).
- **A tee needs its target run first.** The trunk was drawn before the top branch and ended on a bare cell, which the format reads as two runs meeting at grade, not a tee; the hand pass turned the trunk's end into a tee onto the branch. The editor could refuse a run ending on another run's cell, or offer the tee.
- **Save reported an error after a 201.** The plate was created, the page stayed on the editor with the generic message; to be reproduced with a visible tab (the authoring tab was hidden, which throttles the page's timers to one a second and is also why the first attempts looked like the editor hanging).
