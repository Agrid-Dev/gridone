...
```
 y=-25  [trunk top 16] -- PRESSION 18 -- PEC-D2 A (22) ---- [merge top 28]
 y=-18  (16) ------------ PRESSION 18 -- PEC-D2 B (22) ---- (28)
 y=-11  (16) ------------ PRESSION 18 -- PEC-D3 A (22) ---- (28)
 y= -4  (16) ------------ PRESSION 18 -- PEC-D3 B (22) ---- (28)
...
- **Two links per change-over block.** A `link` has one inlet and one outlet a cell apart; the cold supply joins the hot supply riser and the cold return leaves the hot return riser two columns away, so one link would cross a riser. Each leg has its own link, as the bays' cold water has. The five links to the cold production (the balance line at the vessel and the four change-over legs) lead to the fourth plate, and the fourth plate's balance line leads back here; the committed files name no target, since a synoptic id is the instance's, so the two are pointed at each other on the instance once both plates are pushed. A push replaces the whole document, so every push of either file sets its links' `synoptic_id` back to null and the two are pointed at each other again after it.
...
- **A symbol's label takes no part in the readout search.** It sits at a fixed lift above the body, so it lands on a run even where every chip and panel is kept clear: fifteen of this plate's labels cross a run's centreline (seven on each bay), `CPT-EC-ECH-04` and `ECH EC04` among them, where the district primary runs alongside itself. Logged as an open point of the visual language.
- **The plate is 3760 × 1821 px**, seven tenths again the bays'; the first view is the whole plate, small, and the operator zooms (AGR-1164's call on the first plate holds).
...
## The fourth plate: `synoptic/production-froid.json`

The old GTB drawing "PRODUCTION FROID" is the hot production's template with cold names and less on it: the district primary on the plate exchanger ECH EG04 with the control valve and the heat meter on the primary return, the air separator and one twin pump PEG-E2 on the secondary, a supply collector feeding three circuits (change-over VCV RdC, CTA / radiateur / RAC, change-over VCV chambres), a return collector, the dirt separator and the expansion vessel VEC 04 on the return. The change-over circuits are plain circuits here: their crossover valves and cold legs are drawn on the hot view only, so this plate draws none and the hot plate's links lead to it. One pipe the drawing does not draw is on the plate: the balance line at VEC 04, which the hot view draws arriving in its return as "Depuis Production EG", leaves this plate's vessel riser into the "VERS PRODUCTION EC" link, so the two productions click through to each other both ways and the one pipe they share has one direction. A link is an off-page connector, the same shape whether or not the other page exists (Decision 13), and the plates place one only behind a pipe that leaves a drawing; this one is the sibling drawing's. The grid is the third plate's with two branches instead of four (rows -11 and -4) and three circuits (columns 32, 44, 56); every tag sits where the third plate's does, the two pressure tags on the branches at column 18 as the hot plate's four now do.

### Bindings inventory

| Element | Slot / tag | Device | Attribute | Format |
|---|---|---|---|---|
| `pompe-peg-e2-a`, `pompe-peg-e2-b` | `state`, `speed` | `schneider_pompe_peg_e2a` / `_e2b` | `onoff_state`, `speed` | MARCHE / ARRÊT; tr/min, 0 |
| pipes `peg-e2-*-branch` | `flow` | same | `onoff_state` | bool |
| `pression-peg-e2-a`, `pression-peg-e2-b` | value | same | `head` | bar, 1; head B reads the drive's 65.535 sentinel, raw, until the driver decodes it to null (AGR-1367) |
| `tt-primaire-depart`, `tt-primaire-retour` | value | `schneider_heat_meter_eg` | `tmpdepart`, `tmpretour` | °C, 1 |
| `cpt-eg-ech-04` | `energy` | `schneider_heat_meter_eg` | `energie` | Wh, 0, by the hot meter's inference. The register has read 339 169 984 since June while the meter's power and flow move every poll; displayed raw, as the hot plate's is, and a frozen register is the driver's to explain, not a plate's to hide behind a marker (AGR-1366). Nothing on the plate says it is frozen: staleness ages from `last_updated`, which every poll stamps, not from `last_changed`, so a driver re-writing the same number never goes stale |
| `tt-secondaire-depart` | value | `schneider_prod_divers` | `echeg_tmpdepart` (AI1) | °C, 1 |
| `tt-manque-eau` | value | same | `eg04_defmanqueeau` (BI4) | NORMAL / DÉFAUT |
| `pot-a-boue` | `fault` | same | `eg04_defpotboue` (BI6) | NORMAL / DÉFAUT |
| `v-primaire` | `position` | none | "Signal 0 %": no device exposes the valve output | `text`, "non mesurée" |
| `tt-cta-*`, `tt-vcv-*-retour` (4) | value | none | no device reads these circuit temperatures | `text`, "non mesurée" |
| `cpt-vcv-rdc`, `cpt-cta`, `cpt-vcv-chambres` | `energy` | none | CPT-EG-VCO / CTA / CHAMBRES, on no device since the controller split | `text`, "non mesurée" |
| `v-cta` | `state` | none | no end switch for this circuit | `text`, "non mesurée" |
| `tt-vcv-rdc-depart`, `tt-vcv-chambres-depart`, `v-vcv-rdc`, `v-vcv-chambres` | value, `state` | not yet | `vc_tmpdepart` / `vcv_tmpdepart` and the `vceg_*` / `vcveg_*` end switches read, but which is RdC and which chambres is unanswered | `text`, "non identifiée" |

Dropped as on the third plate: the meter's `puissance`, `debit`, `volume`, `deltat`; the pumps' flow, current, hours, starts, frequency, warnings and control registers; the pump electricity meter; the sub-station's alarms and discordances; everything of the hot production; the AHU energy valves.

### What authoring in the editor taught

The measurement the issue asked for (AGR-1166), 2026-09-18 and 19, agent time: preflight 25 min as a delta on the third plate's; editor pass about 35 min, of which 15 were spent on pixel clicks through the browser extension and 20 on driving the editor's own handlers by script (place, draw, bind, save) once the tab's timer throttling was understood; hand pass 3 min (readable ids, twelve tags, the title, the description, one tee); the two test tables, the plate's own test file, the live check in the app and this section 20 min; a code-review fix pass 30 min (the balance line's direction, the pressure change moved to the hot plate's branch, the doc's counts, the shared test helpers). About 1 h 55 against the third plate's 3 h 10, with the third plate's grid rules already written and its layout copied, and no render-and-diagnose round at all: the renderer's spec took the plate on the first run.

- **The editor authored 21 symbols, 15 runs, 5 attribute bindings, 8 text markers, 3 device ids and 2 flows, and the API took the document first time.** (The committed file carries 6 attribute bindings and 7 markers on its symbols: the meter's register was a marker in the editor and was bound in the second review pass, by hand.) Every waypoint the hand-written third plate carries, the editor produced from the same clicks, overhead crossings included; the collector port form and the attribute picker bound what the hand pass would have typed.
- **What it could not author, added by hand**: pipe tags (the twelve readings, six of them bound), the title label, the description, readable ids (the editor numbers by type: `chilled_supply-5`), and later the meter's register binding (AGR-1370).
- **The plate is 3280 × 1462 px**, the third plate's 3760 × 1821 less two branches and one circuit.
- **A tee needs its target run first.** The trunk was drawn before the top branch and ended on a bare cell, which the format reads as two runs meeting at grade, not a tee; the hand pass turned the trunk's end into a tee onto the branch. The editor could refuse a run ending on another run's cell, or offer the tee (AGR-1368).
- **Save reported an error after a 201.** The plate was created, the page stayed on the editor with the generic message; to be reproduced with a visible tab (the authoring tab was hidden, which throttles the page's timers to one a second and is also why the first attempts looked like the editor hanging) (AGR-1368).
...
**Passages and gates** (`x-passages`, `x-gates-flow` in the published schemas), what the fluid passes between inside each type, for circulation (Decision 14): `heat_pump` return to supply, and it gates; `tank` its primary pair and its domestic pair, apart; `collector` every authored port (`"all"`), for the runs of one fluid; `mixing_valve` its three ports; `link` `in` and `out`; `plate_exchanger` its primary and secondary pairs, apart; `expansion_vessel` none, a dead end. Inline types sit on their run and pass it through; `pump`, `pump_double` and `valve_isolation` gate it, `loop_heater` does not (off, it stops heating, not the loop).
