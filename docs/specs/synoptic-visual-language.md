...
18. **Run state is on the symbol, not only in the panel.** A symbol whose `state` slot is bound draws a 4 px LED after its label: `--status-ok` when the value is an on-state (`MARCHE`, `OUVERTE`), `--muted-foreground` when off, `--status-error` when the device is faulty, fault first. An isolation valve shows its state on the glyph instead, the ISA way: closed is a solid bowtie in `--synoptic-stroke`, open is hollow. The panel LED follows the same rule. Flow animation is not a state signal: it is off under `prefers-reduced-motion` and absent from a screenshot, so a stopped machine must read as stopped without it.
