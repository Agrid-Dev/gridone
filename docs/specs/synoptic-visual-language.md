...
5. **Six surface tokens and no literal**: `--synoptic-plate` (the ground), `--synoptic-grid` (footprints, editor grid), `--synoptic-body`, `--synoptic-body-x`, `--synoptic-body-y` (top face and the two visible side faces of a body, each its own value per theme) and `--synoptic-stroke` (symbol outline). Chips use `--card` / `--border`, text uses `--foreground` / `--muted-foreground`. A face is never a black overlay: in light the sides are darker than the top, in dark the top is lighter than the plate and the sides sit between, so a theme changes three tokens and every face follows. The values are the largest step the dark theme allows before the muted detail stroke sinks into the top face: light 91 / 84 / 76 under a 97 plate, dark 33 / 26 / 21 over an 11 plate. Chosen by review from a comparison of hairline faces, filled faces and filled faces stepped further, drawn on the ECS Est plate.
6. **No gradients, no metal.** Three tiers of line: silhouette 2 px in `--synoptic-stroke`; internal detail 1.25 px (fan, mesh, diaphragm), in `--muted-foreground` so it sits behind the silhouette; operating information (chips, panels, labels) on `--card` with `--foreground` text, the only tier in full contrast. The gallery's `scada-metal` gradient is dropped with the dark-only palette it was drawn for.
...
14. **The equipment panel is the POC's chrome, one slot per row.** 164 px wide; title left, LED right; a rule; then one row per bound slot with the slot label muted left and the value right, `ÉTAT` and `DÉFAUT` first. The POC's one-row `ÉTAT` / `DÉFAUT` header does not hold two values at the 11 px floor, so they are rows like the others; a `DÉFAUT` value on a faulty device is in the error colour. A slot with no binding is not a row (the format's appendix: "a panel with fewer bound rows is a smaller panel"), and a symbol with a single bound reading has no panel at all: the value is a chip under its label. A readout takes the first spot clear of everything already placed (bodies, bars, runs, symbol labels, tags and readouts): a panel above the label, a chip under it, else left of the body, right, below, then the four corners; when the ring is taken the gaps widen and it is tried again, up to three times, and the first spot is the last resort. A panel is joined to the symbol by a 1 px leader ending on the silhouette, and so is a chip the search sent anywhere but under its label (a valve the drawing leaves unnamed has only that chip to say what it shows); a tag's chip takes the same search above or below its run. The editor (AGR-1165) may later let the author move it.
...
18. **Run state is on the symbol, not only in the panel.** A symbol whose `state` slot is bound draws a 4 px LED after its label: `--status-ok` when the value is an on-state (`MARCHE`, `OUVERTE`), `--muted-foreground` when off, `--status-error` when the device is faulty, fault first. An isolation valve shows its state on the glyph instead, the ISA way: closed is a solid bowtie in `--synoptic-stroke`, open is hollow, and a valve with no reading (no `state` bound, a marked one, a stale one) is a hollow bowtie in `--muted-foreground`, so a silent valve never reads as open. The panel LED follows the same rule. Flow animation is not a state signal: it is off under `prefers-reduced-motion` and absent from a screenshot, so a stopped machine must read as stopped without it.
...
| `pump` | 1 × 1, inline | circle and triangle on the axis plane, triangle downstream | circle, triangle downstream | `state`, `speed` |
...
| `dirt_separator` | inline | none | circle with a settling cone, dirt collecting at the bottom | `fault` |
| `pump_double` | inline | none | two pumps side by side across the run | `state` |
| `energy_meter` | inline | none | square housing with register window, `kWh` above | `energy` |
| `loop_heater` | inline | none | square housing with the heating element's zigzag | `state`, `fault` |
| `valve_control` | inline | none | the isolation bowtie with an `M` above; its slot is a position, never open or closed, so it always takes the no-reading stroke of Decision 18 | `position` |
