...
13. Servers that refuse to answer unsecured requests at all (an option on some Siemens/CODESYS-class gear) are **not supported**. Connecting to one, even with a pin already on file, still fails: the unsecured `GetEndpoints` discovery leg in Decision 9 runs on every secure connect regardless of pin state, and skipping it would trade away that decision's guarantee — a swapped server certificate would surface as a generic handshake failure instead of a typed `OpcuaSecurityError`. No device in the current fleet needs this, so the guarantee wins. Unsecured (no `security_policy`) transports are unaffected either way — they never enter this discovery/pin path. Revisit if a real target device turns out to require it.
