...
- Preserve-on-blank is likewise done in `DevicesService.update_transport`: a blank value for a declared secret field is dropped from the PATCH patch dict before it reaches `TransportClient.update_config`'s merge, so the stored value survives. A patch reduced to nothing by this filtering is normalized back to `None`, not `{}`, so it doesn't spuriously trigger a reconnect or device restart.
