...
import { Box, Loader2, RefreshCw, Trash2, Upload } from "lucide-react";
...
export function BuildingModelCard({
  ...
  // model uploaded before a converter change needs rebuilding — from the IFC
  // already on the server, not from a second upload.
  const regenerateMutation = useMutation({
    mutationFn: () => client.assets.regenerateModel(assetId),
    onSuccess: (rebuilding) => {
      queryClient.setQueryData(buildingModelKey(assetId), rebuilding);
      toast.success(t("editPage.model.regenerateStarted"));
    },
    onError: (error: Error) => toast.error(error.message),
  });
