...
vi.mock("react-i18next", () =>
  createI18nMock({
    ...
    "editPage.model.regenerate": "Rebuild 3D scene",
    "editPage.model.regenerateStarted": "Rebuilding the 3D scene.",
...
describe("BuildingModelCard", () => {
  ...
  it("rebuilds the scene from the stored file, without a second upload", async () => {
    const model = makeModel();
    mockClient.assets.getModel.mockResolvedValue(model);
    mockClient.assets.regenerateModel.mockResolvedValue({
      ...model,
      status: "processing",
    });
    const user = userEvent.setup();
    ...
    await user.click(
      await screen.findByRole("button", { name: "Rebuild 3D scene" }),
    );

    await waitFor(() =>
      expect(mockClient.assets.regenerateModel).toHaveBeenCalledExactlyOnceWith(
        "b1",
      ),
    );
    expect(mockClient.assets.uploadModel).not.toHaveBeenCalled();
    expect(mockToast.success).toHaveBeenCalledWith("Rebuilding the 3D scene.");
  });

  it("hides the rebuild action from a viewer", async () => {
    mockClient.assets.getModel.mockResolvedValue(makeModel());
    renderCard({ canWrite: false });

    expect(await screen.findByText("3D scene ready")).toBeInTheDocument();
    expect(
      screen.queryByRole("button", { name: "Rebuild 3D scene" }),
    ).not.toBeInTheDocument();
  });
