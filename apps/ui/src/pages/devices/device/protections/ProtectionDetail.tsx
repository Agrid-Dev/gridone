...
import type { Protection } from "@gridone/sdk";
...
function Retirement({
  ...
  return (
    <form onSubmit={state.submit} className="space-y-4" noValidate>
      <ProtectionSummary rule={state.rule} catalog={catalog} />
      <FieldShell
      ...
      <ProtectionError error={state.reload.error ?? state.retire.error} />
...
function History({ id, catalog }: { id: string; catalog: PointCatalog }) {
  const { t, i18n } = useTranslation("protections");
  const { data: history } = useProtectionHistory(id);
  return (
...
export default function ProtectionDetail() {
  ...
  const {
    data: { protection: rule, reasons },
  } = useProtection(device.id, protectionId);
  ...
  return (
    <section className="space-y-6">
        ...
        actions={
          ...
          !rule.retirement && (
            <>
              ...
              <Button asChild>
                <Link to={`${protectionPath(rule)}/edit`}>{t("edit")}</Link>
              </Button>
            </>
          )
        }
      ...
      <div className="rounded-xl border bg-card p-5">
        <ProtectionSummary rule={rule} catalog={catalog} />
      </div>
      {rule.retirement && (
