...
import { isConflict } from "./useOperatingRules";

export function OperatingRuleDiagnostics({
  reasons,
}: {
  reasons?: WriteReason[];
}) {
  const { i18n } = useTranslation("operatingRules");
...
export function OperatingRuleError({ error }: { error: unknown }) {
  const { t } = useTranslation("operatingRules");
  if (!error) return null;
  if (isGridoneError(error) && error.reasons?.length)
    return <OperatingRuleDiagnostics reasons={error.reasons} />;
  return (
