...
describe("parseBuildingScene", () => {
  it("collects storeys sorted by index with their spaces", () => {
    ...
    expect([...parsed.spaceStoreys]).toEqual([["sp-1", "st-0"]]);
  });
  ...
  it("handles a scene without storeys", () => {
    ...
    expect(parsed.spaceStoreys.size).toBe(0);
  });
});
...
describe("pickSpaceId", () => {
  const spaceStoreys = new Map([["sp-1", "st-0"]]);

  it("resolves a hit to its room in 3D, and ignores the massing", () => {
    const { spaceMesh } = makeScene();
    const plan = { planish: false, planStoreyId: null, spaceStoreys };
    expect(pickSpaceId(spaceMesh, plan)).toBe("sp-1");
    expect(pickSpaceId(new Mesh(), plan)).toBeNull();
    expect(pickSpaceId(null, plan)).toBeNull();
  });

  it("lets only the plan's own storey answer while plan-ish", () => {
    const { spaceMesh } = makeScene();
    expect(
      pickSpaceId(spaceMesh, {
        planish: true,
        planStoreyId: "st-0",
        spaceStoreys,
      }),
    ).toBe("sp-1");
    // A room of the storey hidden above the plan reads as empty air.
    expect(
      pickSpaceId(spaceMesh, {
        planish: true,
        planStoreyId: "st-1",
