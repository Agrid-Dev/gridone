// Throwaway verification harness for AGR-1319: mounts the real protections
...
import type { Device, Protection } from "@gridone/sdk";
import { GridoneClientProvider } from "@/contexts/GridoneClientContext";
import { AuthProvider } from "@/contexts/AuthContext";
import { DeviceProvider } from "@/contexts/DeviceContext";
import { TooltipProvider } from "@/components/ui";
import DeviceProtections from "@/pages/devices/device/protections";
import schemas from "@/pages/devices/device/protections/__fixtures__/schemas.json";
import "@/index.css";
import "@/i18n";
...
const rules: Record<string, Protection> = {
  r1: flowRule,
  r2: brokenRule,
  r3: retiredRule,
};
...
const client = {
  me: async () => ({
    ...
    permissions: ["protections:write", "protections:read", "devices:read"],
  }),
  ...
  protections: {
    list: async () => [
      { protection: flowRule, reasons: [] },
      {
        protection: brokenRule,
        reasons: [{ code: "protection_reference_invalid" }],
      },
      { protection: retiredRule, reasons: [] },
    ],
    get: async (id: string) => ({
      protection: rules[id] ?? flowRule,
      reasons: id === "r2" ? [{ code: "protection_reference_invalid" }] : [],
    }),
        ...
        <AuthProvider>
          <DeviceProvider>
            <TooltipProvider>
              <MemoryRouter
                initialEntries={[
                  `/devices/${chiller.id}/config/protections${route}`,
                ]}
              >
