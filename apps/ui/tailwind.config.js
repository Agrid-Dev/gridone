/** @type {import('tailwindcss').Config} */
module.exports = {
  darkMode: ["class"],
  content: ["./index.html", "./src/**/*.{js,ts,jsx,tsx}"],
  theme: {
    extend: {
      fontFamily: {
        sans: ["Figtree", "system-ui", "-apple-system", "sans-serif"],
        display: ["Outfit", "system-ui", "sans-serif"],
      },
      colors: {
        border: "hsl(var(--border))",
        input: "hsl(var(--input))",
        ring: "hsl(var(--ring))",
        background: "hsl(var(--background))",
        foreground: "hsl(var(--foreground))",
        primary: {
          DEFAULT: "hsl(var(--primary))",
          foreground: "hsl(var(--primary-foreground))",
        },
        secondary: {
          DEFAULT: "hsl(var(--secondary))",
          foreground: "hsl(var(--secondary-foreground))",
        },
        destructive: {
          DEFAULT: "hsl(var(--destructive))",
          foreground: "hsl(var(--destructive-foreground))",
        },
        success: {
          DEFAULT: "hsl(var(--success))",
          foreground: "hsl(var(--success-foreground))",
        },
        warning: {
          DEFAULT: "hsl(var(--warning))",
          foreground: "hsl(var(--warning-foreground))",
        },
        muted: {
          DEFAULT: "hsl(var(--muted))",
          foreground: "hsl(var(--muted-foreground))",
        },
        accent: {
          DEFAULT: "hsl(var(--accent))",
          foreground: "hsl(var(--accent-foreground))",
        },
        popover: {
          DEFAULT: "hsl(var(--popover))",
          foreground: "hsl(var(--popover-foreground))",
        },
        card: {
          DEFAULT: "hsl(var(--card))",
          foreground: "hsl(var(--card-foreground))",
        },
        sidebar: {
          DEFAULT: "hsl(var(--sidebar))",
          foreground: "hsl(var(--sidebar-foreground))",
          accent: "hsl(var(--sidebar-accent))",
        },
        // Semantic palette — see index.css and lib/semanticColors.ts.
        "status-ok": "hsl(var(--status-ok))",
        "status-info": "hsl(var(--status-info))",
        "status-warning": "hsl(var(--status-warning))",
        "status-error": "hsl(var(--status-error))",
        "status-ok-foreground": "hsl(var(--status-ok-foreground))",
        "status-info-foreground": "hsl(var(--status-info-foreground))",
        "status-warning-foreground": "hsl(var(--status-warning-foreground))",
        "status-error-foreground": "hsl(var(--status-error-foreground))",
        "hvac-heat": "hsl(var(--hvac-heat))",
        "hvac-cool": "hsl(var(--hvac-cool))",
        "hvac-fan": "hsl(var(--hvac-fan))",
        "hvac-dry": "hsl(var(--hvac-dry))",
        "hvac-auto": "hsl(var(--hvac-auto))",
        water: "hsl(var(--water))",
        "node-decision": "hsl(var(--node-decision))",
        "node-command": "hsl(var(--node-command))",
        "node-write": "hsl(var(--node-write))",
        "node-notify": "hsl(var(--node-notify))",
        "synoptic-plate": "hsl(var(--synoptic-plate))",
        "synoptic-grid": "hsl(var(--synoptic-grid))",
        "synoptic-body": "hsl(var(--synoptic-body))",
        "synoptic-body-x": "hsl(var(--synoptic-body-x))",
        "synoptic-body-y": "hsl(var(--synoptic-body-y))",
        "synoptic-stroke": "hsl(var(--synoptic-stroke))",
        "synoptic-edge": "hsl(var(--synoptic-edge))",
        "synoptic-metal-hi": "hsl(var(--synoptic-metal-hi))",
        "synoptic-metal": "hsl(var(--synoptic-metal))",
        "synoptic-metal-lo": "hsl(var(--synoptic-metal-lo))",
        "synoptic-brass-hi": "hsl(var(--synoptic-brass-hi))",
        "synoptic-brass-lo": "hsl(var(--synoptic-brass-lo))",
        "synoptic-vessel-hi": "hsl(var(--synoptic-vessel-hi))",
        "synoptic-vessel-lo": "hsl(var(--synoptic-vessel-lo))",
        "synoptic-dark": "hsl(var(--synoptic-dark))",
        "synoptic-slab-hi": "hsl(var(--synoptic-slab-hi))",
        "synoptic-slab-lo": "hsl(var(--synoptic-slab-lo))",
        "synoptic-slab-edge": "hsl(var(--synoptic-slab-edge))",
        "synoptic-reading": "hsl(var(--synoptic-reading))",
        "fluid-primary-supply": "hsl(var(--fluid-primary-supply))",
        "fluid-primary-return": "hsl(var(--fluid-primary-return))",
        "fluid-heating-supply": "hsl(var(--fluid-heating-supply))",
        "fluid-heating-return": "hsl(var(--fluid-heating-return))",
        "fluid-dhw": "hsl(var(--fluid-dhw))",
        "fluid-dhw-loop": "hsl(var(--fluid-dhw-loop))",
        "fluid-cold-water": "hsl(var(--fluid-cold-water))",
        "fluid-chilled-supply": "hsl(var(--fluid-chilled-supply))",
        "fluid-chilled-return": "hsl(var(--fluid-chilled-return))",
        "fluid-condenser-supply": "hsl(var(--fluid-condenser-supply))",
        "fluid-condenser-return": "hsl(var(--fluid-condenser-return))",
      },
      borderRadius: {
        "2xl": "calc(var(--radius) + 8px)",
        xl: "calc(var(--radius) + 4px)",
        lg: "var(--radius)",
        md: "calc(var(--radius) - 2px)",
        sm: "calc(var(--radius) - 4px)",
      },
      keyframes: {
        "accordion-down": {
          from: { height: 0 },
          to: { height: "var(--radix-accordion-content-height)" },
        },
        "accordion-up": {
          from: { height: "var(--radix-accordion-content-height)" },
          to: { height: 0 },
        },
        "highlight-fade": {
          "0%": { backgroundColor: "hsl(var(--primary) / 0.2)" },
          "100%": { backgroundColor: "transparent" },
        },
        "fade-up": {
          from: { opacity: 0, transform: "translateY(8px)" },
          to: { opacity: 1, transform: "translateY(0)" },
        },
        // Odometer pair: the outgoing value rises out of the frame while the
        // incoming one climbs in from below. See components/layout/RollingText.
        "roll-out": {
          from: { opacity: 1, transform: "translateY(0)" },
          to: { opacity: 0, transform: "translateY(-100%)" },
        },
        "roll-in": {
          from: { opacity: 0, transform: "translateY(100%)" },
          to: { opacity: 1, transform: "translateY(0)" },
        },
        // Both ends pinned: a piece sets its own `stroke-dashoffset` to line
        // its dash up with its run's when the motion is reduced, and the
        // animation must not start from it (see `Pipe`'s `flowDelay`).
        flow: {
          from: { strokeDashoffset: "0" },
          to: { strokeDashoffset: "-52" },
        },
        blink: { "0%, 55%": { opacity: 1 }, "56%, 100%": { opacity: 0.3 } },
      },
      animation: {
        "accordion-down": "accordion-down 0.2s ease-out",
        "accordion-up": "accordion-up 0.2s ease-out",
        "highlight-fade": "highlight-fade 3s ease-out forwards",
        "fade-up": "fade-up 0.4s ease-out both",
        "roll-out": "roll-out 0.35s ease-out forwards",
        "roll-in": "roll-in 0.35s ease-out both",
        flow: "flow 1.2s linear infinite",
        blink: "blink 1.1s step-end infinite",
      },
    },
  },
  plugins: [require("tailwindcss-animate")],
};
